...
function ProjectileClass(start, targetId, img, type, damage, speed, tier, rotates, parent, owner) {
    // positions
    this.start = start;
    this.x = start.x;
    this.y = start.y;
    this.owner = owner;
    this.id = PROJECTILE_ID[this.owner]++;
    this.parentId = parent; // which tower fired

    this.angle = 0;
    this.targetId = targetId;
    this.target = monsterList[this.owner][targetId];
    this.type = type;
    this.visible = true;
    this.damage = damage;
    this.speed = speed;
...
ProjectileClass.prototype.move = function() {
    ...
    if(this.visible) {
        ...
        if(monsterList[this.owner][this.targetId]) {
            if(this.hitTarget()) {
                ...
                if(this.target.alive) {
                    ...
                    if(monsterList[this.owner][this.targetId].hitWithProjectile(this.damage)) {
                        StateController.notifyTowerKilledMonster(this.parentId, this.owner, type);
                    }
                }
                        ...
                        for(var col = thisTile.col - this.splashRange; col <= thisTile.col + this.splashRange; col++) {    
                            ...
                            if(targetTile.hasMonsters()) {
                                Object.keys(targetTile.monstersOnTile).forEach(
                                    ((monster) => {
                                        var type = monsterList[this.owner][monster].type;
                                        if(monsterList[this.owner][monster] !== undefined) {
                                            if(monsterList[this.owner][monster].hitWithProjectile(this.damage * this.splashRatio)) {
                                                StateController.notifyTowerKilledMonster(this.parentId, this.owner, type);
                                            }
                                        }
                                    })
...
ProjectileClass.prototype.draw = function() {
    if(this.visible) {
        drawBitmapCenteredWithRotation(this.img, this.x, this.y, this.angle); // randomize angle
        if(this.rotates) {
...
ProjectileClass.prototype.die = function() {
    if(this.type === PROJECTILE_TYPE_2 && this.target) {
        // explode!
        makeAnimation(EXPLOSION, this.target.x + TILE_W / 2, this.target.y + TILE_H / 2, 0);
    }
    delete projectileList[this.owner][this.id];
}

function StraightProjectileClass(start, targetPoint, img, type, damage, speed, hits, tier, rotates, parent, owner) {
    ProjectileClass.call(this, start, -1, img, type, damage, speed, tier, rotates, parent, owner);
    this.hits = hits;
    ...
    if(tile.hasMonsters()) {
        ...
        Object.keys(tile.monstersOnTile).forEach(
            ((monster) => {
                ...
                if(monsterList[this.owner][monster] !== undefined) {
                    var type = monsterList[this.owner][monster].type;
                    if(monsterList[this.owner][monster].hitWithProjectile(this.damage)) {
                        StateController.notifyTowerKilledMonster(this.parentId, this.owner, type);
                    }
                    this.hits--;
                }
...
function PortalClass(start, targetId, state, img, type, damage, speed, tier, rotates, parent, owner) {
    ProjectileClass.call(this, start, targetId, img, type, damage, speed, tier, rotates, parent, owner);

    this.state = state;
    this.currTile = StateController.currLevel.tiles[start.row][start.col];
...
PortalClass.prototype.move = function() {
    if(this.state === PORTAL_CIRCLING) {
        ...
        this.target = monsterList[this.owner][this.targetId];
        if(this.target === undefined) {
        ...
        if(this.currTile.hasMonsters()) {
            Object.keys(this.currTile.monstersOnTile).forEach(
                ((monster) => {
                    ...
                    if(monsterList[this.owner][monster] !== undefined && monsterList[this.owner][monster].alive) {
                        // deal damage, teleport if still alive
                        var obj = monsterList[this.owner][monster];
                        if(obj.hitWithProjectile(this.damage)) {
                            StateController.notifyTowerKilledMonster(this.parentId, this.owner, obj.type);
                        } else {
                            this.teleportMonster(monsterList[this.owner][monster].id);
                        }
                        this.capacity--;
                    }
                })
...
PortalClass.prototype.teleportMonster = function(monsterId) {
    // debugger;
    let monster = monsterList[this.owner][monsterId];
    if(this.pointInMonsterPath === null) {
        console.log("SEE PROJECTILE 370!");
        return;
    }

    let newIndex = Math.min(this.pointInMonsterPath + this.teleportDistance, fullMonsterPath[this.owner].length - 1);
    let newTile = fullMonsterPath[this.owner][newIndex].tile;
    let newPixel = gridToPixel(newTile.row, newTile.col);

    // notify tiles
    StateController.currLevel.tiles[monster.currTile.row][monster.currTile.col].notifyMonsterDepart(monster.id);
    StateController.currLevel.tiles[newTile.row][newTile.col].notifyMonsterArrive(monster.id);

    monster.x = newPixel.x;
    monster.y = newPixel.y;
    monster.currTile = newTile;

    monster.pathPosition = fullMonsterPath[this.owner][newIndex].position;
}
...
PortalClass.prototype.becomeStationary = function() {
    ...
    this.currTile = StateController.currLevel.tiles[gridPos.row][gridPos.col];

    // calculate where in the monsterPath you are
    for(var i = 0; i < fullMonsterPath[this.owner].length; i++) {
        let tile = fullMonsterPath[this.owner][i].tile;
...
PortalClass.prototype.draw = function() {
    drawBitmapCenteredWithRotation(this.img, this.x + TILE_W / 2, this.y + TILE_H / 2, this.angle);
}
