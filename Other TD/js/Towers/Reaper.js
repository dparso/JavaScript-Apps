...
function ReaperClass(type, owner) {
                    ...
                    if(obj !== undefined && obj.alive) {
                        if(obj.hitWithProjectile(dmg)) {
                            ...
                            if(obj.towersWithDots[this.id] === undefined) {
                                var dot = new DoTClass(monster, dmg * reaper_dot_ratio, reaper_dot_rates[this.tier + 1], reaper_dot_durations[this.tier + 1], this.id, this.owner);
                                obj.dots[dot.id] = dot;
