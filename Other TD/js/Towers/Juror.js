...
function JurorClass(type, owner) {
...
JurorClass.prototype.attack = function() {
    ...
    for(var target = 0; target < this.targets.length; target++) {
        if(this.targets[target]) {
            var projectile = new ProjectileClass({x: this.x, y: this.y}, this.targets[target].id, projectilePics[this.type][0], this.type, dmg, projectileSpeeds[this.type], this.tier, false, this.id, this.owner);
...
//     // is locally set
//     if(this.type === SOLAR_PRINCE && this.owner === PLAYER) console.log(this.target);
