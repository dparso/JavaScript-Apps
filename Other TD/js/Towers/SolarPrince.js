// solar prince
function SolarPrinceClass(type, owner) {
    ...
    SOLAR_PRINCE_UNIQUE[this.owner] = 1;
}
...
SolarPrinceClass.prototype.draw = function() {
    if(this.visible) {
        ...
        ctx.save();
        ctx.translate(leftX + 3 * leftImg.width / 2, leftY);
        ctx.rotate(Math.cos(this.leftWingAngle - Math.PI) / wingSpeed);
        ctx.drawImage(leftImg, -leftImg.width, -leftImg.height / 2);
        ctx.restore(); // undo translate

 

        ctx.save();
        ctx.translate(rightX + rightImg.width / 2, rightY);
        ctx.rotate(Math.cos(this.rightWingAngle - 2 * Math.PI) / wingSpeed);
        ctx.drawImage(rightImg, 0, -rightImg.height / 2);
        ctx.restore(); // undo translate

        ctx.drawImage(this.img, this.x - this.img.width / 2 + 1, this.y - this.img.height / 2 - TILE_H - 2);
        ctx.restore(); // undo blur & alpha
...
SolarPrinceClass.prototype.attack = function() {
    ...
    for(var degree = -this.attackAngle; degree <= this.attackAngle; degree += 5) {
        ...
        var xOff = Math.cos(rad) * canvas.width; 
        var yOff = Math.sin(rad) * canvas.width;
...
SolarPrinceClass.prototype.drawLightning = function(from, to, maxDiff, strength) {
    ...
    render(from, to, direction, maxDiff, segmentHeight, strength, "#ffd402");
}