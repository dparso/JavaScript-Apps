...
function ConduitClass(type, owner) {
        ...
        Object.keys(tile.monstersOnTile).forEach(
            ((monster) => {
                if(!first) {
                    ...
                    this.targets.push(monsterList[this.owner][Number(monster)]);
                    jumps++;
                } else {
                    first = false;
                }
...
ConduitClass.prototype.attack = function() {
    ...
    for(var chain = 0; chain < this.targets.length; chain++) {
        ...
        if(this.targets[chain].hitWithProjectile(dmg * Math.pow(this.dampening, chain))) {
            StateController.notifyTowerKilledMonster(this.id, this.owner, type);
        }     
    }
}
...
ConduitClass.prototype.drawLightning = function(from, to, maxDiff, strength) {
    ...
    render(from, to, direction, maxDiff, segmentHeight, strength, lightBlue);
}


const minSegmentHeight = 5;
const roughness = 2;
function render(source, target, direction, maxDifference, segmentHeight, count, color) {
    if(count <= 0) return;
    ...
    ctx.stroke();
    ctx.restore();
    requestAnimationFrame(function() {render(source, target, direction, maxDifference, segmentHeight, --count, color);});
}
